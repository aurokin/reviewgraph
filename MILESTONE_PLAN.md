...
- Current execution status as of 2026-05-07 after starting AUR-221: `AUR-244`, `AUR-218`, `AUR-217`, `AUR-219`, `AUR-243`, and `AUR-220` are `Done`; `AUR-221` is `In Progress`; remaining active implementation issues are `Backlog`; `AUR-261` is the milestone gate.
- Active implementation issues:
  ...
  - `AUR-221` / `RG-032: Reconcile Embedded ReviewGraph Markers` / `In Progress`
