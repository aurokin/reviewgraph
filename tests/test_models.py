...
@pytest.mark.parametrize(
    ...
    [
        ...
        lambda: FinalizationStatus(FinalizationState.FAILED_CLOSED, None, "sha256:target"),
        lambda: FinalizationStatus(
        ...
        lambda: TargetFreshnessTransportSummary("wrong", False),
        lambda: TargetFreshnessTransportSummary("pull_request_target", True),
        lambda: TargetFreshnessTransportSummary(
            "pull_request_target",
            True,
            reason_code=TargetFreshnessReasonCode.FORBIDDEN,
        ),
        lambda: TargetFreshnessTransportSummary("pull_request_target", False, request_id="ghp_secret"),
        lambda: TargetFreshnessCheckResult(GateStatus.UNKNOWN),
        lambda: TargetFreshnessCheckResult(GateStatus.PASS),
        lambda: TargetFreshnessCheckResult(
            GateStatus.FAIL,
            reason_code=TargetFreshnessReasonCode.TARGET_MISMATCH,
            transport_summary=TargetFreshnessTransportSummary("pull_request_target", False),
        ),
