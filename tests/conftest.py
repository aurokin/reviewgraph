...
def pytest_collection_modifyitems(config: pytest.Config, items: list[pytest.Item]) -> None:
    ...
    skip_live_llm = pytest.mark.skip(reason="set REVIEWGRAPH_LIVE_LLM=1 to run live LLM smoke tests")
    ...
    for item in items:
        ...
        if "live_llm" in item.keywords and os.environ.get("REVIEWGRAPH_LIVE_LLM") != "1":
