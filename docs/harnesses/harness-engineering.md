...
- Real writer harnesses must keep normal marker reconciliation finalization-owned while testing writer-local recovery for ambiguous accepted-write POST outcomes. Recovery tests should supply only finalized writer input plus recovery comment-scan transport/configuration, prove no second POST happens before a complete marker rescan, and prove incomplete or failed recovery scans return fail-closed or retryable-unknown evidence.
