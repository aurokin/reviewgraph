...
- Adapter boundary harnesses treat `execute_*` callables as the structured adapter boundary. Raw fixture lookup helpers may return raw output, but the graph-facing boundary must return `ReviewerResult` and must validate context package, graph-owned run key, selected reviewer identity, active stage, and capability policy before any lookup or execution. Live LLM policy and live adapter boundaries must run the same package/run-key/capability preflight and reject stale policy results with a mismatched package fingerprint, missing current-ledger reservation, or mismatched reservation proof before provider execution.
...
- Live LLM adapter harnesses are fake-provider by default. They must prove fake reviewers remain default, `--live-llm`/run-level opt-in is required separately from config and transport, provider/model/budget settings are recorded in run identity, provider-bound request text is the approved policy text, live results expose typed redacted `LiveLLMReviewerEvidence`, raw provider responses are not persisted, provider failure reasons map into required/optional reviewer semantics, malformed live output fails without a repair LLM call, retry attempts use distinct run keys and reservations, total timeout caps retry attempts, and `live_llm` smoke tests are skipped unless `REVIEWGRAPH_LIVE_LLM=1`.
...
- Non-interactive post harnesses must prove dry-run bypasses the post gate, public CLI still exposes no `--post` flag, the harness-only post attempt records `post_interaction_gate` and `non_interactive_post_mode`, approval/final-payload sentinels are not called, and writer call count remains zero.
