...
- `tests/test_github_pagination.py` covers fake pagination for files, issue comments, review comments, reviews, and thread state. It must assert exact resource call order, cursor propagation, complete coverage, page-failure fail-closed state, page diagnostics, orphan review-comment thread-state gaps, safe untrusted defaults, and that pagination completes before truncation.
