...
- Live LLM adapter harnesses are fake-provider by default. They must prove fake reviewers remain default, `--live-llm`/run-level opt-in is required, provider/model/budget settings are recorded in run identity, provider-bound request text is the approved policy text, live results expose typed redacted `LiveLLMReviewerEvidence`, raw provider responses are not persisted, malformed live output fails without a repair LLM call, retry attempts use distinct run keys and reservations, and `live_llm` smoke tests are skipped unless `REVIEWGRAPH_LIVE_LLM=1`.
